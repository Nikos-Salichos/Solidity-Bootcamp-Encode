...
class StakeTable extends React.Component<any, any> {
  ...
  renderRow() {
    ...
    return this.state.stakes.map((stake: any, index: any) => {
      ...
      const claimTime = unixTime.toNumber() + this.state.claimTime.toNumber();
      const claimDate = new Date(claimTime * 1000);
      return (
        <Table.Row key={index}>
          {stake[3] == 0 ? null : (
            <>
              <Table.Cell>{stake[0].toString()}</Table.Cell>
              <Table.Cell>{stake[3].toString()} NES</Table.Cell>
              <Table.Cell>{unixTime == 0 ? 0 : date.toUTCString()}</Table.Cell>
              <Table.Cell>{claimDate.toUTCString()}</Table.Cell>
